...
train = pd.read_csv('pm.csv',names=["Date&Time","username","tweet"])
train['actual_tweet']= train['tweet']
# initial b removal
train['tweet'] = train['tweet'].apply(ast.literal_eval).str.decode("utf-8")
train['username']=train['username'].apply(ast.literal_eval).str.decode("utf-8")
#print(train['tweet'])
#RT@ removal
...
train['username']=train['username'].apply(lambda x: " ".join(re.sub("b^\s+|(@[A-Za-z0-9]+)|([^0-9A-Za-z \t])|(\w+:\/\/\S+)","",x).split()))
#print(train['tweet'])
train['actual_tweet']= train['tweet']
...
print(train[['tweet','sentiment']].head(10))
train[['username','actual_tweet','sentiment']].to_csv('sentiments_pm.csv', encoding='utf-8', index=False)
